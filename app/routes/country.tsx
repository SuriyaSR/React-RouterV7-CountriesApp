...
export async function clientLoader({params}: Route.LoaderArgs) {
  const res = await fetch(`https://restcountries.com/v3.1/name/${params.countryName}?fullText=true`);
  if (!res.ok) {
    throw new Response("Failed to fetch countries", { status: res.status });
  }
